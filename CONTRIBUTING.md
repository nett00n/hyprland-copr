...
scripts/gen-report.py              # renders build report from build-report.db (--format github|copr|full-report, --output FILE)
scripts/db-artifacts.py            # build-report.db disk usage report / prune / reset / forget
...
build-report.db                    # sqlite build state (gitignored, generated by full-cycle/stage-*)
...
Force the mock/copr stages to re-run for one or more packages, for `FEDORA_VERSION`'s target:

```shell
make build-pop PKG=pkg1,pkg2    # force re-run for specific packages
make build-pop PKG=""            # force re-run for ALL packages (requires confirmation)
```

This sets `force_run` on the mock/copr rows in `build-report.db`; the next `full-cycle` run
picks it up and clears it (one-shot). Useful after fixing issues without editing the DB by hand.
...
- **build_db.py** — sqlite storage for build-report.db: run history, per-(package, stage, target) results, artifact ledger
...
- **paths.py** — canonical path constants (`SOURCES_DIR`, `BUILD_DB`, etc.) and helpers (`mock_chroot()`, `resolve_target()`)
- **pipeline.py** — build pipeline caching helpers (queries build_db by `target`)
...
- **yaml_utils.py** — loading/saving packages.yaml, filtering, release auto-increment, `prepare_stage()` boilerplate helper
...
The `gen-report.py` script generates markdown reports from `build-report.db`, for `FEDORA_VERSION`'s
target. It supports multiple output formats and COPR status polling:
...
- `--skip-copr-poll` — Skip polling COPR status (use cached status from build-report.db)

By default, `gen-report.py` automatically polls COPR for the status of in-progress builds and updates
`build-report.db` with the latest state. Use `--skip-copr-poll` when running multiple report generations
...
To force a stage to re-run for one or more packages, without a full pipeline run:

```shell
make build-pop PKG=hyprland    # sets force_run on hyprland's mock+copr rows
```

For a stage other than mock/copr, or without going through Make, use `db-shell`:
...
**Rules:**
- Setting `force_run` on any stage forces that stage and all downstream stages to re-run
...
- After a stage executes (success or fail), `force_run` is automatically cleared
  (operator must re-set it to force again; one-shot behavior)
- The `reason` column is automatically set on each stage row to explain why it was cached,
...
where `target` is the mock chroot (e.g. `fedora-44-x86_64`) — building for a second Fedora version
does not overwrite the first. Three tables: `runs` (one row per `full-cycle`/`stage-*` invocation),
`stage_results` (per-package per-stage state), `artifacts` (paths/sizes of build outputs, for disk
cleanup — see below). `make db-shell` opens an interactive sqlite3 shell for ad-hoc inspection.

### Artifact Tracking

Every SRPM, vendor tarball, and mock-built RPM (plus mock logs) is recorded in `build-report.db`'s
`artifacts` table as it's produced:
...
`db-prune` only prunes `srpm`/`rpm`/`vendor` kinds — logs are never touched by it; use
`make clean-logs` for those. Both targets must run in-container (`$(CONTAINER_PYTHON)`): recorded
paths are container-absolute and only resolve with the same volumes mounted.

`make clean-logs` clears `stage_results` and `runs` but **keeps** the artifact ledger — dropping it
would orphan every tracked file with no record of what it is. For a full wipe (irreversible,
confirmation required): `make db-nuke`.

### Running individual pipeline stages

```shell
# Run stages individually (each reads/writes build-report.db)
...
# build-report.db has no stored hashes for this package
